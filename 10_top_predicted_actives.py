"""
10_top_predicted_actives.py
============================
Generates a table of the top predicted active compounds from the test set,
ranked by conformal prediction confidence score.

This answers the key reviewer question: "What does your model actually
output for a practising chemist?"

Method:
  - Retrain best model (RF + RDKit2D) on 80% train split
  - Get predict_proba scores for 20% test split
  - Filter to compounds the model predicts as "active" (prob > 0.5)
  - Rank by probability (confidence) descending
  - Annotate with known true labels (TP vs FP)
  - Output CSV + formatted table

Output:
  results/top_predicted_actives.csv
  figures/top_actives_chart.png
"""
...
# Add SMILES and ChEMBL ID if available
...
# Add standard value if available
...
# ── Filter: predicted actives only, rank by confidence ────────────────────
...
# ── Figure: Confidence bar chart for top 15 ───────────────────────────────
...
plt.close()
print("  ✓ figures/top_actives_chart.png")
print()
