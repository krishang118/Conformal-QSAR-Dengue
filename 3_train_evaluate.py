...
Trains 4 classifiers × 4 fingerprints = 16 combinations.
5-fold stratified cross-validation. Reports AUC-ROC, AUC-PR, MCC.
All models use class_weight='balanced' to handle 71/29 class imbalance.

Output:
  results/results_table.csv   — main benchmark table (Table 2 in paper)
  results/best_model.pkl      — best model for SHAP + conformal prediction
  figures/benchmark_heatmap.png
"""
...
# ── Model definitions (all with class_weight='balanced') ──────────────────
print("\n" + "=" * 60)
...
MODELS = {
    ...
    'XGBoost': XGBClassifier(
        ...
        scale_pos_weight=(1-y).sum() / y.sum(),   # XGB uses this instead of class_weight
...
scorers = {
    ...
    'MCC':     make_scorer(matthews_corrcoef),   # uses predict (binary labels)
}
...
# ── Train best model on FULL dataset (for SHAP + conformal) ───────────────
print("\n" + "=" * 60)
...
# Also always train XGBoost on ECFP4 (used for primary SHAP analysis)
...
# Always train XGBoost on RDKit descriptors (for SHAP descriptor analysis)
...
# Save best combo info for other scripts
...
print("  ✓ results/best_combo.json")

print()
print("✓ Stage 3 complete. Run 4_shap_analysis.py next.")
