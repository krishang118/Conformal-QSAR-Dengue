"""
12_applicability_domain.py
===========================
Applicability Domain (AD) analysis using Tanimoto-based nearest-neighbour distance.

OECD recommends AD analysis for all QSAR models submitted for regulatory or
scientific use. Pairs perfectly with conformal prediction — both address the
question: "When should we trust this model's predictions?"

Method (Tanimoto k-NN AD):
  - Compute ECFP4 fingerprints for all compounds
  - For each test compound, find Tanimoto similarity to k=5 nearest train neighbours
  - AD threshold: mean(train-train similarity) - Z*std(train-train similarity)
  - Compounds with max_sim_to_train < threshold → outside AD (unreliable prediction)

Output:
  results/ad_results.json
  figures/ad_analysis.png
"""
...
# ── Compute ECFP4 for Tanimoto distance ───────────────────────────────────
...
k = 5
test_max_sims = []   # max Tanimoto similarity of each test compound to training set
test_mean_k_sims = []
...
# ── AD threshold: train-train similarity distribution ─────────────────────
print("  Computing train-train similarity for AD threshold (sampled)...")
# Sample 300 train compounds to avoid O(n²) computation
...
Z = 0.5   # conservative Z-score threshold
ad_threshold = train_train_sims.mean() - Z * train_train_sims.std()
ad_threshold = max(ad_threshold, 0.30)  # floor at 0.30
...
# ── Classify test compounds: inside vs. outside AD ─────────────────────────
