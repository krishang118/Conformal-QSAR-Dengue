...
9_roc_curves.py
===============
Generates ROC and Precision-Recall curve plots for the top 5 models.

IMPORTANT: Uses IDENTICAL model hyperparameters and CV setup as
3_train_evaluate.py (n_estimators=300, random_state=42, same skf)
so that curve AUCs match Table 1 exactly.

Legend labels are hard-set to the canonical Table 1 values from
results/results_table_with_mlp.csv to guarantee consistency.

Top 5 models (from Table 1):
  1. RF + RDKit2D       AUC-ROC 0.9487 ± 0.0071  AP 0.6515
  2. XGBoost + RDKit2D  AUC-ROC 0.9460 ± 0.0085  AP 0.6667
  3. LightGBM + RDKit2D AUC-ROC 0.9449 ± 0.0109  AP 0.6683
  4. XGBoost + AtomPair AUC-ROC 0.9389 ± 0.0170  AP 0.6415
  5. RF + ECFP4         AUC-ROC 0.9267 ± 0.0276  AP 0.6290
...
# ── Canonical Table 1 values (hard-coded for legend labels) ───────────────
# These are the authoritative numbers from results_table_with_mlp.csv
CANONICAL = {
...
# ── Define top 5 models — IDENTICAL hyperparams to 3_train_evaluate.py ─────
scale_pos = (y == 0).sum() / y.sum()   # = 13.4
...
# ── CV setup — IDENTICAL to 3_train_evaluate.py ────────────────────────────
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

# ── Collect mean ROC and PR curves via CV ──────────────────────────────────
...
for name, model, X, color in models_config:
    ...
    # Use canonical Table 1 values for the legend labels
    c = CANONICAL[name]
...
print()
print(f"  Random baseline precision: {random_baseline:.3f} ({random_baseline:.1%})")
print(f"  Best model AP (Table 1): {max(v['ap'] for v in CANONICAL.values()):.4f}")
print()
print("✓ Stage 9 complete. All legend values now match Table 1 exactly.")
