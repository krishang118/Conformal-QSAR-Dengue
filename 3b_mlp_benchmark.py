...
3b_mlp_benchmark.py
===================
Adds a Feed-Forward Neural Network (MLP) as the 5th model to the benchmark.

Purpose: Show that classical ML (RF, XGB, LGB, SVM) outperforms a simple deep
learning model on this small, imbalanced dataset — a well-established finding
in low-data QSAR regimes (Ramsundar et al., DeepChem; Wu et al., MoleculeNet 2018).

Runs 5-fold stratified CV for MLP on all 4 fingerprints, then appends results
to the existing results_table.csv and regenerates the benchmark figures.

Output:
  results/results_table_with_mlp.csv   (20 rows: 16 classical + 4 MLP)
  figures/benchmark_heatmap_mlp.png
  figures/benchmark_barplot_mlp.png
"""
...
def make_mlp():
    return MLPClassifier(
        ...
        activation='relu',
        solver='adam',
        alpha=1e-3,              # L2 regularisation (important for small data)
        batch_size=64,
...
for fp_name, X in fingerprints.items():
    ...
    for fold, (train_idx, test_idx) in enumerate(cv.split(X, y)):
        ...
        # Sample weights to handle 13:1 class imbalance
...
plt.close()
print("  ✓ figures/benchmark_barplot_mlp.png")

print()
print("✓ Stage 3b complete. MLP benchmark added.")
print("  Use results_table_with_mlp.csv as the definitive Table 2 for the paper.")
