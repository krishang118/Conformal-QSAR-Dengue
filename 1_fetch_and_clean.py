"""
1_fetch_and_clean.py
====================
Stage 1: Data Acquisition and Cleaning

Pulls dengue virus NS2B-NS3 protease bioactivity data from ChEMBL,
filters and cleans it, applies activity labeling, and saves the
clean dataset ready for featurization.

Target: CHEMBL5980 — Dengue virus type 2 Genome polyprotein (NS2B-NS3 protease focus)
Note  : CHEMBL5980 contains mixed assays (NS2B-NS3 protease, NS5-MTase, NS3 helicase).
        We filter to NS2B-NS3 serine protease assays only before modelling.
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
TARGET_ID              = 'CHEMBL5980'  # Dengue virus type 2 — Genome polyprotein (contains NS2B-NS3)
ACTIVITY_THRESHOLD_NM  = 10_000        # IC50 ≤ 10 µM → active (standard in dengue QSAR literature)
...
# --- Method A: chembl_downloader (SQL) ---
try:
...
# --- Method B: chembl-webresource-client ---
...
# Drop rows missing SMILES or value
smiles_col = 'canonical_smiles'
df = df.dropna(subset=['standard_value', smiles_col])
print(f"  After dropping missing values: {len(df)} records")

# ─────────────────────────────────────────────────────────────────────────────
# STEP 2b: Remove assay contamination (NS5-MTase capping enzyme + NS3 helicase)
# CHEMBL5980 = Genome polyprotein; activities include NS5 capping enzyme assays
# from PubChem HTS (AID 504767) and NS3 helicase — must be excluded for a
# clean NS2B-NS3 protease QSAR model.
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
...
if 'assay_description' in df.columns:
    ...
    # Keep only genuine NS2B-NS3 serine protease assays
    is_protease = (
...
# Group by canonical SMILES, take median IC50 and resolve activity tie
...
# Resolve activity from median IC50 (more robust than voting on individual measurements)
df_dedup['activity']  = (df_dedup['ic50_median'] <= ACTIVITY_THRESHOLD_NM).astype(int)
df_dedup['pIC50']     = -np.log10(df_dedup['ic50_median'] * 1e-9)  # convert nM → M → pIC50
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 6: Lipinski drug-likeness filter (flag, don't remove)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 8: Print full summary (Table 1 data)
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
...
print("✓ Stage 1 complete. Run 2_featurize.py next.")
