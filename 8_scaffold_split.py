...
8_scaffold_split.py
===================
Scaffold-based train/test split sensitivity analysis.

Uses Butina clustering (RDKit) to group compounds by scaffold similarity,
then splits clusters into train/test ensuring no scaffold leakage.
Reruns best model (RF + RDKit2D) and compares AUC-ROC to stratified split.

This preempts the most common reviewer request at JCIM / Digital Discovery.

Output:
  results/scaffold_split_results.json
  figures/scaffold_split_comparison.png   (Figure 8 / Supplementary)
"""
...
X = data['rdkit']     # Best fingerprint (RDKit2D physicochemical)
y = data['y']
...
# ── Step 1: Compute ECFP4 fingerprints for Butina clustering ──────────────
...
# ── Step 3: Scaffold-based 80/20 split ───────────────────────────────────
...
# Handle any valid_idx not in clusters (edge case)
...
# ── Step 4: Train RF on scaffold split ───────────────────────────────────
...
# ── Step 5: Stratified AUC-ROC (canonical from Stage 3 benchmark) ─────────
# Load directly from Stage 3 results to ensure figure == paper text.
# Running an independent CV here would give a slightly different value
# due to RF stochasticity, creating figure/text inconsistency.
...
conclusion = (
    "Scaffold split confirms model generalises across chemical scaffolds."
    if scaffold_auc >= 0.85 else
    "Scaffold split reveals some degree of scaffold memorisation — state as limitation."
)
...
plt.close()
print("  ✓ figures/scaffold_split_comparison.png")
print()
